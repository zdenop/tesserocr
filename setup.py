...
# must be importerd after setuptools
...
_TESSERACT_MIN_VERSION = "3.05.00"
...
def get_tesseract_version():
    ...
    if sys.platform == "win32":
        ...
        if len(tess_lib) >= 1:
            ...
            error = "Tesseract {} library not found in LIBPATH: {}".format(
                lib_version, libpaths
            )
        ...
        # add dlls
        dll_paths = os.getenv("PATH", None)
        if dll_paths:
            dll_paths = dll_paths.split(";")
        else:
            dll_paths = []
        tess_dll = find_library(tess_lib + ".dll", dll_paths)
        if tess_dll:
            tess_dll_dir = os.path.dirname(tess_dll[0])
            for dll in glob.glob(os.path.join(tess_dll_dir, "*.dll")):
                destination = os.path.join("tesserocr", os.path.basename(dll))
                if not os.path.exists(destination):
                    shutil.copy(dll, "tesserocr")
...
def make_extension():
    ...
    return Extension(
        "tesserocr.tesserocr",
        sources=["tesserocr/tesserocr.pyx"],
        language="c++",
        **build_args
    )
...
setup(
    name="tesserocr",
    version=find_version("tesserocr", "__init__.py"),  # 'tesserocr/tesserocr.pyx'
    ...
    packages=find_packages(),  # [ 'tesserocr' ],   # where='src'
    package_dir={
        "tesserocr": "tesserocr",
        "tesserocr.tests": "tesserocr/tests",
        # 'tesserocr' : '.',
    },
    # package_data={
    # 'tesseroct': ['tesserocr/*.dll'],
    # },
    cmdclass={"build_ext": my_build_ext},
    ext_modules=[make_extension()],
    test_suite="tesserocr.tests",
